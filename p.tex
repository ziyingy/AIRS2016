
%%%%%%%%%%%%%%%%%%%%%%% file typeinst.tex %%%%%%%%%%%%%%%%%%%%%%%%%
%
% This is the LaTeX source for the instructions to authors using
% the LaTeX document class 'llncs.cls' for contributions to
% the Lecture Notes in Computer Sciences series.
% http://www.springer.com/lncs       Springer Heidelberg 2006/05/04
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


\documentclass[runningheads,a4paper]{llncs}
\usepackage{amssymb}
\setcounter{tocdepth}{3}
\usepackage{multirow}
\usepackage{graphicx}
\usepackage{epsfig}
\usepackage{xcolor}
\usepackage{times}
\usepackage{graphicx}
\usepackage{mwe}    % loads »blindtext« and »graphicx«
\usepackage{subfig}
\usepackage[numbers,sort]{natbib}
\renewcommand{\bibname}{References}
\renewcommand{\bibsection}{\subsection*{References}~\\[-8ex]}
\usepackage{booktabs}
\usepackage[hyphens]{url}
\renewcommand{\ttdefault}{cmtt}

\urldef{\mailsa}\path|{,|
\urldef{\mailsb}\path|,|
\urldef{\mailsc}\path| }@unimelb.edu.au|    
\newcommand{\keywords}[1]{\par\addvspace\baselineskip
\noindent\keywordname\enspace\ignorespaces#1}

\input{macros.tex}

\begin{document}

\mainmatter  % start of an individual contribution

% first the title is needed
\title{How Precise Does Document Scoring Need To Be?}

% a short form should be given in case it is too long for the running head
\titlerunning{Can Ties in Runs be Exploited?}

% the name(s) of the author(s) follow(s) next
%
%
\author{Ziying Yang \qquad Alistair Moffat \qquad Andrew Turpin}%
%
\authorrunning{How Precise Does Scoring Need To Be?}
% (feature abused for this document to repeat the title also on left hand pages)

% the affiliations are given next; don't give your e-mail address
% unless you accept that it will be published
\institute{
Department of Computing and Information Systems,\\
The University of Melbourne, Australia\\
%% \mailsa\\
%% \mailsb\\
%% \mailsc\\
}


\toctitle{How Precise Does Document Scoring Need To Be?}
\tocauthor{submitted for anonymous review}
\maketitle

\input{sec0-abstract.tex}
\input{sec1-intro.tex}
\input{sec2-ties.tex}
\input{sec3-trecimpact.tex}
\input{sec4-roundingoff.tex}
\input{sec5-conclusion.tex}



\begin{small}
\renewcommand{\bibsep}{2.5pt}
%\bibliographystyle{llncs2e/splncs03}
\bibliographystyle{splncsnat}
\bibliography{strings-shrt,local}
\end{small}

\end{document}
